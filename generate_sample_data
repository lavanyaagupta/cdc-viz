...
def make_places_sample(counties: pd.DataFrame) -> pd.DataFrame:
  ...
  for cond in conditions:
    base = {"Diabetes":11, "Obesity":33, "Coronary Heart Disease": 6, "Asthma":9, "High Blood Pressure": 32, "Depression": 30,}[cond]
    df[f"{cond}_pct"] = np.clip(np.random.normal(loc = base, scale = base * 0.25, size = len(df)), 1, 60).round(1)
    df["total_population"] = np.random.randint(8_000, 950_000, size=len(df))
    return df

def make_hrsa_sample(counties: pd.DataFrame) -> pd.DataFrame:
  df = counties[["counties_fips", "state", "county_name"]].copy()
  df["hpsa_score"] = np.random.randint(0,26,size=len(df))
  df["designation_type"] = np.where(df["hpsa_score"] >= 14, "High Need", np.where(df["hpsa_score"] >= 7, "Moderate Need", "Low Need"))
  return df
...
if __name__ = "__main__":
  ...
  places.to_csv("data.places_sample.csv", index = False)
  hrsa.to_csv("data/hrsa_sample.csv", index = False)
